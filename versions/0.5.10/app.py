...
from rounded_ui import RoundedButton, RoundedEntry
...
class App(tk.Tk):
    ...
    def _style(self):
        ...
        style.configure('TCombobox', padding=8, relief='flat', borderwidth=1, fieldbackground=WHITE)
    ...
    def _show_access_fallback(self, error=''):
        ...
        if auth.has_users():
            ...
            user_entry = RoundedEntry(card, textvariable=username)
            user_entry.pack(fill='x', pady=(0, 10))
            tk.Label(card, text='Senha', bg=WHITE, fg=TEXT).pack(anchor='w', pady=(0, 4))
            pass_entry = RoundedEntry(card, textvariable=password, show='•')
            ...
            RoundedButton(card, text='Entrar', command=login, bg=PINK, fg=WHITE,
            ...
            for label, variable, mask in fields:
                ...
                RoundedEntry(card, textvariable=variable, show=mask).pack(fill='x', pady=(0, 8))
            ...
            RoundedButton(card, text='Criar conta principal', command=create_owner, bg=PINK, fg=WHITE,
                      activebackground=PINK, activeforeground=WHITE, bd=0, pady=10,
    ...
    def show_first_owner_setup(self):
        ...
        RoundedButton(panel, text='Criar conta principal', command=save_owner, bg=PINK, fg='white',
    ...
    def _labeled_entry(self, parent, label, variable, show=None):
        ...
        entry = RoundedEntry(parent, textvariable=variable, show=show)
        entry.pack(fill='x')
        return entry
    ...
    def _nav_button(self, parent, label, command):
        ...
        def draw(hover=False):
            ...
            button.delete('all')
            # Pontos repetidos mantêm as laterais retas e arredondam apenas os cantos.
            button.create_polygon(12, 0, 12, 0, 196, 0, 196, 0, 208, 12, 208, 12,
                                  208, 30, 208, 30, 196, 42, 196, 42, 12, 42, 12, 42,
                                  0, 30, 0, 30, 0, 12, 0, 12, smooth=True, splinesteps=16,
                                  fill=fill, outline='')
    ...
    def _layout(self):
        ...
        self._nav_buttons = {}
        for text, cmd in nav:
            self._nav_button(self.sidebar, text, cmd)
        ...
        RoundedButton(bottom, text='Sair', command=self.logout, bg=NAVY2, fg='white', bd=0,
    ...
    def header(self, title, subtitle=''):
        if title in self._nav_buttons:
            self._active_nav = title
            for draw in self._nav_buttons.values():
                draw()
    ...
    def show_products(self):
        ...
        if self.can_manage_catalog():
            RoundedButton(actions, text='Importar Excel', command=self.import_excel, bg=NAVY2, fg='white', bd=0,
                      padx=14, pady=8, cursor='hand2').pack(side='left', padx=4)
            RoundedButton(actions, text='+ Nova mercadoria', command=self.new_product_dialog, bg=NAVY, fg='white', bd=0,
                      padx=14, pady=8, cursor='hand2').pack(side='left', padx=4)

        searchf = tk.Frame(self.content, bg=BG)
        searchf.pack(fill='x', padx=30, pady=(0, 12))
        self.search_var = tk.StringVar()
        entry = RoundedEntry(searchf, textvariable=self.search_var)
        entry.pack(side='left', fill='x', expand=True)
        entry.bind('<KeyRelease>', lambda e: self.fill_products())
        RoundedButton(searchf, text='Exportar PDF', command=self.export_pdf, bg='white', fg=NAVY, bd=1,
    ...
    def new_product_dialog(self):
        ...
        def add_field(key, label, row, col, colspan=1):
            ...
            var = tk.StringVar()
            vars_[key] = var
            entry = RoundedEntry(box, textvariable=var)
            entry.pack(fill='x')
            return entry
        ...
        for col, (label, var) in enumerate([('Quantidade no depósito', depot), ('Quantidade em exposição', display)]):
            ...
            RoundedEntry(sf, textvariable=var).pack(fill='x')
        ...
        RoundedButton(footer_inner, text='Cancelar', command=win.destroy, bg=WHITE, fg=TEXT,
                  bd=1, relief='solid', padx=18, pady=9, cursor='hand2').pack(side='right', padx=(10, 0))
        RoundedButton(footer_inner, text='Salvar mercadoria', command=save, bg=NAVY, fg='white',
    ...
    def show_inventory(self):
        ...
        search = tk.StringVar()
        RoundedEntry(top, textvariable=search).pack(fill='x')
        box = tk.Frame(self.content, bg=WHITE)
        box.pack(fill='both', expand=True, padx=30, pady=(0, 25))
        ...
        def edit(_event=None):
            ...
            for label, var in [('Quantidade no depósito', depot), ('Quantidade em exposição', display)]:
                ...
                RoundedEntry(win, textvariable=var).pack(fill='x', padx=24, pady=(3, 10))
            ...
            RoundedButton(win, text='Salvar inventário', command=save, bg=NAVY, fg='white', bd=0, pady=9,
    ...
    def show_movements(self):
        ...
        if self.can_edit_stock():
            ...
            search_entry = RoundedEntry(inner, textvariable=search_var)
            ...
            qty_entry = RoundedEntry(inner, textvariable=qty_var, width=10)
            ...
            register_btn = RoundedButton(inner, text='Registrar', command=save, bg=NAVY, fg='white', bd=0,
    ...
    def show_users(self):
        ...
        actions = tk.Frame(head, bg=BG)
        actions.pack(side='right', anchor='e')
        RoundedButton(actions, text='+ Novo usuário', command=self.new_user_dialog, bg=NAVY, fg='white', bd=0,
                  padx=14, pady=8, cursor='hand2').pack(side='left', padx=4)
        box = tk.Frame(self.content, bg=WHITE)
        box.pack(fill='both', expand=True, padx=30, pady=(0, 25))
        ...
        RoundedButton(footer, text='Ativar selecionado', command=lambda: toggle(True), bg=SUCCESS, fg='white', bd=0,
                  padx=12, pady=7, cursor='hand2').pack(side='left', padx=(0, 6))
        RoundedButton(footer, text='Bloquear selecionado', command=lambda: toggle(False), bg=DANGER, fg='white', bd=0,
                  padx=12, pady=7, cursor='hand2').pack(side='left')
        if self.current_user['role'] == 'OWNER':
            RoundedButton(footer, text='Alterar perfil', command=lambda: self.change_user_role_dialog(tree),
    ...
    def change_user_role_dialog(self, tree):
        ...
        RoundedButton(form, text='Salvar perfil', command=save, bg=NAVY, fg='white', bd=0, pady=10,
    ...
    def new_user_dialog(self):
        ...
        for label, var, show in [('Nome completo', full_name, None), ('Usuário', username, None),
            ...
            RoundedEntry(form, textvariable=var, show=show).pack(fill='x')
        ...
        RoundedButton(form, text='Criar usuário', command=save, bg=NAVY, fg='white', bd=0, pady=10,
    ...
    def show_updates(self):
        ...
        RoundedButton(box, text='Verificar atualizações agora', command=self.check_updates_interactive, bg=NAVY, fg='white',
