...
class App(tk.Tk):
    ...
    def show_login(self):
        ...
        status = tk.Label(panel, text='', bg='#073DA8', fg='#FFD6D6', font=('Segoe UI', 9))
        status.pack(anchor='w', pady=(7, 0))

        def login(_event=None):
            user_value = '' if user_state['placeholder'] else username.get()
            pass_value = '' if pass_state['placeholder'] else password.get()
            user = auth.authenticate(user_value.strip(), pass_value)
            if not user:
        ...
        forgot = tk.Label(panel, text='Esqueci minha senha', bg='#073DA8', fg=WHITE,
                          font=('Segoe UI', 10, 'underline'), cursor='hand2')
        forgot.pack(pady=(27, 0))
        forgot.bind('<Button-1>', lambda _event: messagebox.showinfo(
