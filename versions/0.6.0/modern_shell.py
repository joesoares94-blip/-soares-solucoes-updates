...
class ModernMixin:
    def modern_layout(self):
        ...
        try:
            ...
            self.tk.call(crop,'copy',original,'-from',int(w*.21),int(h*.23),int(w*.80),int(h*.79))
            self._brand_image=crop.subsample(max(1,(crop.width()+197)//198))
            tk.Label(brand,image=self._brand_image,bg=NAVY,bd=0).pack(anchor='w')
        except (tk.TclError,OSError):
            label(brand,'SOARES SOLUÇÕES',WHITE,('Segoe UI',16,'bold')).pack(anchor='w',pady=(8,10))
        label(brand,'Controle de estoque','#B3C5D9',('Segoe UI',9)).pack(anchor='w',padx=4,pady=(2,0))
