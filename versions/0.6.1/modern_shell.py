...
class ModernMixin:
    def brand_lockup(self, parent, large=False):
        ...
        try:
            original=tk.PhotoImage(data=base64.b64encode(Path(__file__).with_name('brand_logo.png').read_bytes()))
            crop=tk.PhotoImage()
            w,h=original.width(),original.height()
            self.tk.call(crop,'copy',original,'-from',int(w*.225),int(h*.25),int(w*.415),int(h*.77))
            target=100 if large else 52
            symbol=crop.subsample(max(1,(crop.width()+target-1)//target))
            mark=tk.Label(row,image=symbol,bg=NAVY,bd=0)
            mark.image=symbol
        ...
        name=tk.Frame(row,bg=NAVY);name.pack(side='left')
    ...
    def modern_access_shell(self,title,subtitle):
        ...
        identity=tk.Frame(shell,bg=NAVY);identity.grid(row=0,column=0,sticky='nsew')
        decoration=tk.Canvas(identity,bg=NAVY,bd=0,highlightthickness=0)
        decoration.place(relwidth=1,relheight=1)
        def draw(event):
            decoration.delete('all')
            w,h=event.width,event.height
            for offset in (0,48,96):
                decoration.create_oval(w-220+offset,h-250+offset,w+250+offset,h+220+offset,
                                       outline='#1C4265',width=2)
        decoration.bind('<Configure>',draw)
        message=tk.Frame(identity,bg=NAVY)
        message.place(relx=.5,rely=.43,anchor='center')
        self.brand_lockup(message,large=True).pack(anchor='w',pady=(0,32))
        label(message,'Seu estoque.\nTudo sob controle.',WHITE,('Segoe UI',-32,'bold'),justify='left',wraplength=420).pack(anchor='w')
        label(message,'Organize produtos e acompanhe\nas movimentações da sua empresa.',
              '#B3C5D9',('Segoe UI',-16),justify='left',wraplength=420).pack(anchor='w',pady=(18,0))
        label(identity,'SOARES SOLUÇÕES  •  GESTÃO DE ESTOQUE','#B3C5D9',('Segoe UI',9)).place(relx=.5,rely=.92,anchor='center')
        right=tk.Frame(shell,bg=BG);right.grid(row=0,column=1,sticky='nsew')
        card=Panel(right,padding=30)
        card.place(relx=.5,rely=.5,anchor='center',width=440)
        label(card.body,title,TEXT,('Segoe UI',-28,'bold'),wraplength=368,justify='left').pack(anchor='w')
        label(card.body,subtitle,MUTED,('Segoe UI',10),wraplength=368,justify='left').pack(anchor='w',pady=(8,20))
        self.access_form=tk.Frame(card.body,bg=WHITE);self.access_form.pack(fill='x')
        label(right,f'Soares Soluções  •  Versão {self.APP_VERSION}',MUTED,('Segoe UI',9)).place(relx=.5,rely=.96,anchor='center')
