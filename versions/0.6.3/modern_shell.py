...
class ModernMixin:
    ...
    def modern_access_shell(self,title,subtitle):
        ...
        right=tk.Canvas(shell,bg='#081C58',bd=0,highlightthickness=0)
        right.grid(row=0,column=1,sticky='nsew')
        def draw_right(event):
            right.delete('background')
            w,h=event.width,event.height
            steps=max(1,min(w,240))
            for i in range(steps):
                t=i/max(steps-1,1)
                color=f'#{int(9+4*t):02x}{int(29+91*t):02x}{int(87+153*t):02x}'
                right.create_rectangle(i*w/steps,0,(i+1)*w/steps+1,h,
                                       fill=color,outline=color,tags='background')
            right.tag_lower('background')
        right.bind('<Configure>',draw_right)
        card=tk.Frame(right,bg='#073DA8',highlightthickness=1,highlightbackground='#D8E8FF')
        card.place(relx=.5,rely=.49,anchor='center',width=430,height=535 if 'Configuração' in title else 465)
        body=tk.Frame(card,bg='#073DA8',padx=29,pady=24)
        body.pack(fill='both',expand=True,padx=8,pady=8)
        self.brand_lockup(body,large=False).pack(anchor='w',pady=(0,18))
        label(body,title,WHITE,('Segoe UI',22,'bold'),wraplength=350,justify='left').pack(anchor='w')
        label(body,subtitle,'#D6E8FF',('Segoe UI',9),wraplength=350,justify='left').pack(anchor='w',pady=(5,15))
        self.access_form=tk.Frame(body,bg='#073DA8');self.access_form.pack(fill='x')
