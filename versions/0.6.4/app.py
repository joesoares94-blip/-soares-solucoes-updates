...
class App(ModernMixin, tk.Tk):
    ...
    def _pill_button(self, parent, text, command):
        ...
        def draw(_event=None):
            ...
            # Clip each stripe to the capsule's circular ends.
            for x in range(1,width-1,2):
                t=x/max(width-1,1)
                color=f'#{int(8+28*t):02x}{int(73+86*t):02x}{int(174+75*t):02x}'
                edge=min(x-1,width-2-x)
                inset=27-(max(0,27**2-(27-edge)**2)**.5) if edge<27 else 0
                button.create_rectangle(x,1+inset,min(x+2,width-2),54-inset,
                                        fill=color,outline=color,tags='button')
            self._rounded_shape(button,1,1,width-2,54,27,'',outline='#A9D8FF',tags='button')
