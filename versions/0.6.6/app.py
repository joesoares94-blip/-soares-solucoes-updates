...
from modern_ui import Button, Entry, ScrollArea, configure_style, rounded, gradient_capsule
...
class App(ModernMixin, tk.Tk):
    ...
    def __init__(self):
        if sys.platform == 'win32':
            try:
                # Avoid Windows bitmap-stretching the Tk window at 125%/150%.
        ...
        # Use the available Full HD workspace while allowing a 1366x768 screen.
        self.geometry(f'{min(1920, screen_w)}x{min(1080, max(700, screen_h - 80))}')
    ...
    def _rounded_shape(self, canvas, x1, y1, x2, y2, radius, fill, outline='', width=1, tags=()):
        if outline:
            rounded(canvas, x1, y1, x2, y2, radius, outline, tags)
        if fill:
            inset = width if outline else 0
            rounded(canvas, x1+inset, y1+inset, x2-inset, y2-inset,
                    max(0, radius-inset), fill, tags)
    ...
    def _pill_entry(self, parent, variable, placeholder, icon='user', password=False):
        ...
        row = tk.Canvas(parent, height=54, bg=background, highlightthickness=0, bd=0)
        row.pack(fill='x', pady=(0, 12))
        entry = tk.Entry(row, textvariable=variable, relief='flat', bd=0, bg='#0D347F', fg=WHITE,
        ...
        def draw(_event=None):
            ...
            self._rounded_shape(row, 1, 1, width - 2, 52, 26, '#0D347F', outline='#FFFFFF',
                                width=1, tags='field')
        ...
        row.bind('<Button-1>', lambda _event: entry.focus_set())
    ...
    def _pill_button(self, parent, text, command):
        button = tk.Canvas(parent, height=56, bg=parent.cget('bg'), highlightthickness=0,
                           bd=0, cursor='hand2', takefocus=1)
        button.pack(fill='x', pady=(12, 0))
        def draw(_event=None):
            button.delete('button')
            width = max(button.winfo_width(), 250)
            rounded(button, 1, 1, width-1, 55, 27, '#A9D8FF', 'button')
            # Transparent antialiased edges reveal the thin capsule border.
            gradient_capsule(button, width-4, 50, 25, '#0849AE', '#249FEB', 'button')
            button.move(button.find_withtag('button')[-1], 2, 3)
    ...
    def show_first_owner_setup(self):
        ...
        self._pill_button(panel, 'Criar conta principal', save_owner)

    def _labeled_entry(self, parent, label, variable, show=None):
        parent_bg = parent.cget('bg')
        label_fg = WHITE if parent_bg in {LOGIN_GRADIENT_START, '#073DA8', '#0B2464'} else TEXT
    ...
    def show_login(self):
        ...
        user_entry,user_state = self._pill_entry(panel, username, 'Usuário', icon='user')
        pass_entry,pass_state = self._pill_entry(panel, password, 'Senha', icon='eye', password=True)
        status = tk.Label(panel, text='', bg=panel.cget('bg'), fg='#FFE5E5',
                          font=('Segoe UI', 9), anchor='w', height=1, wraplength=300)
        status.pack(fill='x', pady=(0, 2))

        def login(_event=None):
            user = None if user_state['placeholder'] or pass_state['placeholder'] else auth.authenticate(
        ...
        self._pill_button(panel, 'Entrar', login)
        recovery=tk.Label(panel,text='Esqueci minha senha',bg=panel.cget('bg'),fg=WHITE,
                          cursor='hand2',font=('Segoe UI',10,'underline'))
        recovery.pack(pady=(18,0))
        recovery.bind('<Button-1>',lambda _event: messagebox.showinfo(
            'Recuperação de senha','Peça ao Proprietário ou Administrador do sistema para redefinir sua senha.',
            parent=self))
