...
import services
from modern_ui import Button, Entry, Panel, icon, rounded, shade_band, NAVY, BLUE, BG, TEXT, MUTED, BORDER, WHITE
...
class ModernMixin:
    ...
    def modern_access_shell(self,title,subtitle):
        ...
        try:
            art=tk.PhotoImage(file=str(Path(__file__).with_name('warehouse_login_full.png')))
            identity.art=art
        except (tk.TclError,OSError):
            identity.art=None
        def draw_identity(event):
            ...
            if identity.art:
                source=identity.art
                # Crop the full-resolution image to cover the entire left panel.
                # Tk's integer subsample would make it small and leave blank bands.
                if source.width()<w or source.height()<h:
                    factor=max((w+source.width()-1)//source.width(),
                               (h+source.height()-1)//source.height())
                    source=source.zoom(factor)
                sx=(source.width()-w)//2
                sy=(source.height()-h)//2
                view=tk.PhotoImage(master=identity,width=w,height=h)
                view.tk.call(view,'copy',source,'-from',sx,sy,sx+w,sy+h,'-to',0,0)
                identity.view=view
                identity.create_image(0,0,anchor='nw',image=view)
                top_shade=shade_band(identity,w,top=True)
                bottom_shade=shade_band(identity,w,top=False)
                identity.shades=(top_shade,bottom_shade)
                identity.create_image(0,0,anchor='nw',image=top_shade)
                identity.create_image(0,h-190,anchor='nw',image=bottom_shade)
            identity.create_text(39,35,anchor='nw',text='SOARES SOLUÇÕES',fill='#081D39',
                                 font=('Segoe UI',15,'bold'))
            identity.create_text(38,34,anchor='nw',text='SOARES SOLUÇÕES',fill=WHITE,
                                 font=('Segoe UI',15,'bold'))
            identity.create_text(39,h-85,anchor='nw',text='Tecnologia que simplifica o seu dia a dia.',
                                 fill='#081D39',font=('Segoe UI',15,'bold'))
            identity.create_text(38,h-86,anchor='nw',text='Tecnologia que simplifica o seu dia a dia.',
                                 fill=WHITE,font=('Segoe UI',15,'bold'))
            identity.create_text(39,h-52,anchor='nw',text='Controle seu estoque com clareza e confiança.',
                                 fill='#081D39',font=('Segoe UI',10))
